...
public final class MyEncryptedSharedPreferences implements SharedPreferences {
    ...
    MyEncryptedSharedPreferences(@NonNull String name,
    ...
    public static MyEncryptedSharedPreferences create(@NonNull String fileName,
        ...
        return new MyEncryptedSharedPreferences(fileName, masterKeyAlias,
    ...
    public static final class Editor implements SharedPreferences.Editor {
        ...
        Editor(MyEncryptedSharedPreferences encryptedSharedPreferences,
        ...
        @Override
        @NonNull
        public Editor putString(@Nullable String key, @Nullable String value) {
            if (value == null) {
                value = NULL_VALUE;
            }
        ...
        @Override
        @NonNull
        public Editor putInt(@Nullable String key, int value) {
            ByteBuffer buffer = ByteBuffer.allocate(Integer.BYTES + Integer.BYTES);
        ...
        @Override
        @NonNull
        public Editor putLong(@Nullable String key, long value) {
        ...
        @Override
        @NonNull
        public Editor putFloat(@Nullable String key, float value) {
        ...
        @Override
        @NonNull
        public Editor putBoolean(@Nullable String key, boolean value) {
        ...
        public Editor putByteArray(@Nullable String key, @Nullable byte[] value) {
            ByteBuffer buffer;
            if (value != null) {
                buffer = ByteBuffer.allocate(Integer.BYTES + Integer.BYTES + value.length);
                buffer.putInt(EncryptedType.BYTE_ARRAY.getId());
                buffer.putInt(value.length);
                buffer.put(value);
            } else {
                buffer = ByteBuffer.allocate(Integer.BYTES + Integer.BYTES);
                buffer.putInt(EncryptedType.BYTE_ARRAY.getId());
                buffer.putInt(-1);
            }
            putEncryptedObject(key, buffer.array());
        ...
        @Override
        @NonNull
        public Editor remove(@Nullable String key) {
    ...
    public byte[] getByteArray(@Nullable String key, byte[] defaultValue) {
        Object value = getDecryptedObject(key);
                ...
                switch (type) {
                    ...
                    case BYTE_ARRAY:
                        ...
                        if (arrayLength == -1) {
                            ...
                            byte[] out = Arrays.copyOfRange(buffer.array(), buffer.position(), buffer.position() + arrayLength);
                            Arrays.fill(buffer.array(), (byte) 0);
                            returnValue = out;
                        }
                        break;
