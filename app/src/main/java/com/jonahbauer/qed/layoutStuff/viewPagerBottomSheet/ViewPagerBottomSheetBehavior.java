 ...
 * Adjusted so that touch gets intercepted more appropriate
 */
...
public class ViewPagerBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (mPeekHeightAuto) {
            ...
            mPeekHeight = Math.max(mPeekHeightMin, mParentHeight - mParentWidth * 9 / 16);
        }
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        {
            ...
            if (scroll != null) {
                ...
                ViewGroupUtils.getDescendantRect(parent, scroll, actualPosition);

                DisplayMetrics dm = parent.getResources().getDisplayMetrics();
                final Rect screen = new Rect(20, 20, parent.getWidth() - 20, parent.getHeight() - 20);

                if (!screen.intersect(actualPosition)) invalidateScrollingChild();
            }
        }
        ...
        if (Math.abs(mInitialY - event.getY()) > mViewDragHelper.getTouchSlop() && !mIsHorizontalScroll) {
            mIsVerticalScroll = true;
            mIsHorizontalScroll = false;
        }
        if (Math.abs(mInitialX - event.getX()) > mViewDragHelper.getTouchSlop() && !mIsVerticalScroll) {
        ...
        return action == MotionEvent.ACTION_MOVE &&
                !mIgnoreEvents &&
                !(scroll != null && parent.isPointInChildBounds(scroll, (int) event.getX(), (int) event.getY()) && scroll.canScrollVertically((int) Math.signum(mInitialY - event.getY()))) &&
                mIsVerticalScroll;
    }
    ...
    public void setBottomSheetCallback(BottomSheetCallback callback) {
        Log.w("ViewPagerBottomSheetBehavior",
                "ViewPagerBottomSheetBehavior now supports multiple callbacks. `setBottomSheetCallback()` removes"
    ...
    private int calculatePeekHeight() {
        if (mPeekHeightAuto) {
            return Math.max(mPeekHeightMin, mParentHeight - mParentWidth * 9 / 16);
        }
        return mPeekHeight;
    }
    ...
    boolean shouldHide(View child, float yvel) {
        ...
        mPeekHeight = calculatePeekHeight();
    ...
    View findScrollingChild(View view) {
        ...
        if (view instanceof ViewPager2) {
            ...
            if (adapter instanceof ViewPagerBottomSheetFragmentStateAdapter) {
                FragmentViewHolder viewHolder = ((ViewPagerBottomSheetFragmentStateAdapter) adapter).getFragmentViewHolder(viewPager.getCurrentItem());
    ...
    void dispatchOnSlide(int top) {
        ...
        if (bottomSheet != null && !mCallbacks.isEmpty()) {
            float slideOffset = (top > mMaxOffset)
                    ? (float) (mMaxOffset - top) / (mParentHeight - mMaxOffset)
