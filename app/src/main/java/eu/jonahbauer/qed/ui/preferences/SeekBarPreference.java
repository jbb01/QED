 ...
 */
/*
 * Adjusted to allow external values different from the internal ones
 * The width of the value label is set to the maximum width of the external value strings
 */

package eu.jonahbauer.qed.ui.preferences;
...
public class SeekBarPreference extends Preference {
    ...
    private int[] mExternalValues;
    private String[] mExternalValueStrings;

    public SeekBarPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);

        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference, defStyleAttr, defStyleRes);

        // The ordering of these two statements are important. If we want to set max first, we need
        // to perform the same steps by changing min/max to max/min as following:
        // mMax = a.getInt(...) and setMin(...).
        mMin = a.getInt(R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference_min, 0);
        setMax(a.getInt(R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference_android_max, 100));
        setSeekBarIncrement(a.getInt(R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference_seekBarIncrement, 0));
        mAdjustable = a.getBoolean(R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference_adjustable, true);
        mShowSeekBarValue = a.getBoolean(R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference_showSeekBarValue, false);
        mUpdatesContinuously = a.getBoolean(R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference_updatesContinuously,
                false);

        int externalValuesResId = a.getResourceId(R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference_externalValues, 0);
        int externalValueStringResId = a.getResourceId(R.styleable.eu_jonahbauer_qed_layoutStuff_SeekBarPreference_externalValueStrings, 0);

        int[] externalValues = null;
        String[] externalValueStrings = null;
        if (externalValuesResId != 0) {
            externalValues = a.getResources().getIntArray(externalValuesResId);
        }
        if (externalValueStringResId != 0) {
            externalValueStrings = a.getResources().getStringArray(externalValueStringResId);
        }

        if (externalValues != null && externalValueStrings != null) {
            setExternalValues(externalValues, externalValueStrings);
        } else if (externalValueStrings != null)  {
            setExternalValues(null, externalValueStrings);
        } else if (externalValues != null) {
        ...
        if (mSeekBarValueTextView != null) {
            if (mExternalValueStrings != null) {
                ...
                for (String externalValueString : mExternalValueStrings) {
                    float length = paint.measureText(externalValueString);
                ...
                for (int externalValue : mExternalValues) {
                    float length = paint.measureText(String.valueOf(externalValue));
                    if (length > max) max = length;
                }
                mSeekBarValueTextView.setWidth((int) (max + dp10));
            }
    ...
    public void setExternalValues(int[] externalValues) {
        setExternalValues(externalValues, null);
    }

    public void setExternalValues(int[] externalValues, String[] externalValueStrings) {
        int increment = getSeekBarIncrement();
        if (increment == 0) increment = 1;

        int count = (int) Math.ceil((getMax() - getMin() + 1.) / increment);

        if (externalValues != null) {
            if (externalValues.length != count) throw new IllegalArgumentException();

            this.mExternalValues = externalValues;

            if (externalValueStrings == null) adjustSeekBarValueTextViewWidth();
        }

        if (externalValueStrings != null) {
            if (externalValueStrings.length != count) throw new IllegalArgumentException();

            this.mExternalValueStrings = externalValueStrings;
            adjustSeekBarValueTextViewWidth();
        }
    }
    ...
    private void setValueInternal(int seekBarValue, boolean notifyChanged) {
        ...
        if (seekBarValue != mSeekBarValue) {
            ...
            if (mExternalValues != null) {
                persistInt(mExternalValues[seekBarValue]);
            } else {
    ...
    void updateLabelValue(int value) {
        if (mSeekBarValueTextView != null) {
            if (mExternalValueStrings != null) {
                mSeekBarValueTextView.setText(mExternalValueStrings[value]);
            } else if (mExternalValues != null) {
                mSeekBarValueTextView.setText(String.valueOf(mExternalValues[value]));
            } else {
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        myState.mMin = mMin;
        myState.mMax = mMax;
        myState.mExternalValues = mExternalValues;
        myState.mExternalValueStrings = mExternalValueStrings;
        return myState;
    }
    ...
    private static class SavedState extends BaseSavedState {
        ...
        SavedState(Parcel source) {
            ...
            if (valueSize != -1) {
                mExternalValues = new int[valueSize];
                source.readIntArray(mExternalValues);
            } else {
                mExternalValues = null;
                source.readIntArray(new int[0]);
            }

            int valueStringSize = source.readInt();
            if (valueStringSize != -1) {
                mExternalValueStrings = new String[valueStringSize];
                source.readStringArray(mExternalValueStrings);
            } else {
                mExternalValueStrings = null;
        ...
        public void writeToParcel(Parcel dest, int flags) {
            ...
            dest.writeInt(mExternalValues != null ? mExternalValues.length : -1);
            dest.writeIntArray(mExternalValues);

            dest.writeInt(mExternalValueStrings != null ? mExternalValueStrings.length : -1);
            dest.writeStringArray(mExternalValueStrings);
        }
    }
}
